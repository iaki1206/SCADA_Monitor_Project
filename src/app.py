...
# Fix the import path for PcapManager
import sys
sys.path.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
from src.utils.pcap_manager import PcapManager
...
def toggle_scanning():
    ...
    try:
        ...
        # Emit imediat statusul pentru feedback rapid
        socketio.emit('scanning_status', {
            "scanning": scanning_active,
            "status": status
        }, broadcast=True)  # Adăugat broadcast=True pentru actualizare instantanee
...
def simulate_events():
    while True:
        if not simulation_active:
            time.sleep(0.1)  # Reduced sleep time when inactive
            continue
        ...
        time.sleep(0.5)  # Reduced from 1 to 0.5 for better responsiveness
...
def capture_packets():
    while True:
        ...
        time.sleep(15)  # Reduced from 30 to 15 for better responsiveness
...
def shutdown_server():
    ...
    # Oprește serverul Flask
    func = request.environ.get('werkzeug.server.shutdown')
    if func is None:
        raise RuntimeError('Nu rulează cu serverul Werkzeug')
    func()
    return jsonify({"status": "Server se închide..."})
